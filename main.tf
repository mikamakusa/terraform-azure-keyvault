...
resource "azurerm_key_vault_managed_storage_account" "this" {
  ...
  storage_account_id           = try(element(data.azurerm_storage_account.this.*.id, lookup(var.managed_storage_account[count.index], "storage_account_id")))
...
resource "azurerm_key_vault_managed_storage_account_sas_token_definition" "this" {
  ...
  sas_template_uri           = try(element(data.azurerm_storage_account.this.*.id, lookup(var.managed_storage_account_sas_token_definition[count.index], "sas_template_id")))
